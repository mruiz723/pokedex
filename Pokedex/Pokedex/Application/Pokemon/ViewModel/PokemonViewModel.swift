...
class PokemonViewModel: ObservableObject {
    ...
    func fetchPokemons() {
        isLoadingPage = true
        guard PokemonAPI.offset + PokemonAPI.limit <= PokemonAPI.count, let url = Endpoint.pokemons().url else { return }
    ...
    func fetchMoreListPockemons() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) { [weak self] in
            guard let self = self else { return }
            guard PokemonAPI.offset + PokemonAPI.limit <= PokemonAPI.count else { return }
            var newPokemons: [PKMPokemon] = []
